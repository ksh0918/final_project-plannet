...
public class NotiService {
    ...
    public NotiDTO friendLoad(String id) {
    ...
    public int friendAdd (String id, String keyword){
        // 해당 유저가 없다면 0
        // 친구추가를 할 수 있는 유저라면 1
        // 친구추가가 되어있다면 2
        int result = 0;
            ...
            if(recive == null) return result; // 해당 유저가 없음 0
                ...
                if(id.equals(recive.getId())) {
                    ...
                    if(isFriend == null) { // 친구가 아니라면
                        ...
                        if(alreadySend.size() != 0) result = 3; // 이미 내가 보낸 요청사항이 있음
                        else if(alreadyReceive.size() != 0) result = 4; // 상대가 보낸 요청사항이 있음
    ...
    public boolean notiResponse(Long key, boolean status) {
        try {
            ...
            // 응답을 결정했으니 더이상 알람창에 해당 알림 뜨지 않도록
    ...
    public boolean scalCntCheck(String id) {
        try {
            Member member = memberRepository.findById(id).orElseThrow(EntityNotFoundException::new);
